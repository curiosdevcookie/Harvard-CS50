...
string get_guess(int wordsize) {
  string guess = get_string("Your guess, please: ");
...
int check_word(string guess, int wordsize, int status[], string choice) {
  ...
  // and break so you don't compare that letter further if it's in the word,
  // but not the right spot, score CLOSE point (yellow) keep track of the
  // total score by adding each individual letter's score from above
  for (int i = 0; i < wordsize; i++) {
    if (guess[i] == choice[i]) {
      status[i] = EXACT;
      score += EXACT;
...
void print_word(string guess, int wordsize, int status[]) {
  // Print boxed-in letters for each letter in the word
  for (int i = 0; i < wordsize; i++) {
    switch (status[i]) {
    case EXACT:
